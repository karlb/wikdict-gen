...
class TestParseHTML(unittest.TestCase):
    def test_entity(self):
        self.assertEqual(
            ...
            "die Art und Weise des Herabhängens von Stoffen o.\xa0Ä.",
        )

    def test_subscript(self):
        self.assertEqual(
            html_parser.parse(
                "Gruppenformel CH<sub>3</sub>–(CH<sub>2</sub>)<sub>8</sub>–</small/>COOH"
            ),
            "Gruppenformel CH₃–(CH₂)₈–COOH",
        )

    def test_ref(self):
        self.assertEqual(
            html_parser.parse(
                'Beschlag aus Holz, Knochen oder Metall<ref name="Grabungswörterbuch">Grabungswörterbuch, Stichwort [http://ausgraeberei.de/woerterbuch/index.html?Infodeu/Riemenzunge.htm Riemenzunge]</ref> am (herabhängenden<ref name="TemporaNostra">Tempora Nostra: Mode im Hochmittelalter, Lexikon [http://www.gewandung.de/gewandung/index.php?id=lx_riemenzunge&kontextId=178&kontextNav=1 Riemenzunge]</ref>) Ende eines Gürtels, zur Verstärkung<ref name="Grabungswörterbuch" /> und Beschwerung<ref name="TemporaNostra" />'
            ),
...
class TestParseCleanup(unittest.TestCase):
    def test_bold_and_italics(self):
        ...
        # wiki syntax
        self.assertEqual(
            clean_wiki_syntax("Brunnen mit ''Spring''fontänen"),
            "Brunnen mit Springfontänen",
        )
        self.assertEqual(
            clean_wiki_syntax("'''V'''ereinte '''N'''ationen"), "Vereinte Nationen"
        )
        # keep single ticks
        self.assertEqual(clean_wiki_syntax("Karl's test case"), "Karl's test case")

    def test_noise_at_start(self):
        self.assertEqual(clean_wiki_syntax(": Gesamtheit, alle "), "Gesamtheit, alle")
